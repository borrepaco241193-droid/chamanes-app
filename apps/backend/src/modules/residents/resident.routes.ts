    ...
    async (req, reply) => {
      ...
      const [communityUsers, total] = await Promise.all([
        fastify.prisma.communityUser.findMany({
          ...
          include: {
            user: {
              select: { id: true, firstName: true, lastName: true, email: true, phone: true, avatarUrl: true, idVerificationStatus: true, idPhotoUrl: true },
            },
            unitResidents: {
              where: { moveOutDate: null },
              include: {
  ...
  fastify.post<{ Params: { communityId: string }; Body: unknown }>(
    ...
    async (req, reply) => {
      ...
      if (existingUser) {
        ...
        userId = newUser.id
        // Return temp password in response so admin can share it with the resident
        if (autoGenerated) {
      ...
      if (isPrivilegedRole) {
        const otp = randomInt(100000, 1000000).toString() // 6 digits, cryptographically secure
